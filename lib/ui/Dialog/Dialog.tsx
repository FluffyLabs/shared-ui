...
const Dialog: React.FC<DialogProps> = ({ className, children, asChild }) => {
	...
	return (
		<Comp
			className={cn(
				`flex flex-col w-full h-full bg-card pb-3 rounded-lg border-1`,
				className,
			)}
		>
			{children}
...
const DialogContent = ({
	...
	return (
		<Comp
			className={cn(
				`flex flex-col px-7 pt-[30px] h-full overflow-auto text-secondary-foreground text-sm`,
				className,
			)}
		>
			{children}
...
const DialogFooter = ({
	...
	return (
		<>
			...
			<Comp className={cn("m-6 mb-7 flex justify-end", className)}>
				{children}
			</Comp>
		</>
	);
...
const DialogWithCompoundComponents = Dialog as React.FC<DialogProps> &
	CompoundComponents;
